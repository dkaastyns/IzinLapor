    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex flex-col">
                ...
                <h1 class="text-xl font-extrabold text-gray-900 tracking-tight">{{ t('manage_complaints') }}</h1>
            </div>
        </template>
        ...
        <div class="max-w-7xl mx-auto space-y-6">
            ...
            <Transition name="net-err">
                <div v-if="overdueCount > 0" class="flex items-center gap-3 p-4 rounded-2xl border bg-gradient-to-r from-amber-50 to-orange-50 border-amber-200 text-amber-800 animate-fade-in-up shadow-sm">
                    ...
                    <div class="flex-1">
                        <p class="font-bold text-sm">{{ t('overdue_warning') }}</p>
                        <p class="text-xs mt-0.5 text-amber-700" v-html="`Ada <strong>${overdueCount} laporan</strong> yang melewati estimasi penyelesaian.`"></p>
                    </div>
                </div>
            </Transition>
            ...
            <Transition name="net-err">
                <div v-if="offlineToast" class="flex items-center gap-3 p-4 rounded-2xl border bg-red-50 border-red-200 text-red-700 animate-fade-in-up">
                    <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-red-400 to-rose-500 flex items-center justify-center shadow-sm flex-shrink-0">
                        <svg class="w-5 h-5 text-white animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 5.636a9 9 0 010 12.728M15.536 8.464a5 5 0 010 7.072M12 12h.01M8.464 8.464a5 5 0 000 7.072M5.636 5.636a9 9 0 000 12.728" /></svg>
                    </div>
                    <div class="flex-1">
                        <p class="font-bold text-sm">Tidak ada koneksi internet</p>
                        <p class="text-xs mt-0.5">
                            <template v-if="offlineToast === 'update'">Perubahan status tidak dapat disimpan saat offline.</template>
                            <template v-else>Laporan tidak dapat dihapus saat offline.</template>
                        </p>
                    </div>
                    <button @click="offlineToast = null" class="p-2 hover:bg-red-100 rounded-lg transition flex-shrink-0">
            ...
            <div class="glass-card premium-panel p-5 animate-fade-in-up">
                <div class="flex flex-col lg:flex-row gap-4">
                    <div class="flex-1 relative group">
                        <svg class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 group-focus-within:text-primary-500 transition-colors duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        >
                            <button v-if="searchQuery" @click="searchQuery = ''" class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-red-500 hover:bg-red-50 p-1.5 rounded-lg transition-all">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12" /></svg>
                            </button>
                        </Transition>
                    </div>
                    <div class="w-full lg:w-52">
                        <select v-model="filterCategory" class="input-premium py-3.5 text-sm">
            ...
            <div class="glass-card premium-panel overflow-hidden animate-fade-in-up min-h-[400px]" style="animation-delay: 0.05s;">
                ...
                <div v-else-if="localComplaints.length > 0 || isSearching">
                    ...
                    <div class="relative overflow-x-auto min-h-[300px]">
                        ...
                        >
                            <div v-if="isSearching" class="absolute inset-0 z-20 bg-white/40 backdrop-blur-[2px] flex items-center justify-center">
                        ...
                        <table class="table-premium w-full">
                            <thead>
                                <tr>
                                    <th class="tbl-th">{{ t('table_id') }}</th>
                                    <th class="tbl-th">{{ t('table_reporter') }}</th>
                                    <th class="tbl-th">{{ t('table_title') }}</th>
                                    <th class="tbl-th hidden md:table-cell">{{ t('table_category') }}</th>
                                    <th class="tbl-th hidden lg:table-cell">{{ t('table_location') }}</th>
                                    <th class="tbl-th">{{ t('table_status') }}</th>
                                    <th class="tbl-th hidden xl:table-cell">{{ t('table_estimation') }}</th>
                                    <th class="tbl-th hidden lg:table-cell">{{ t('table_rating') }}</th>
                                    <th class="tbl-th hidden md:table-cell">{{ t('table_date') }}</th>
                                    <th class="tbl-th text-right">{{ t('table_action') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr
                                ...
                                >
                                    <td class="tbl-td font-mono text-xs text-primary-500 font-bold">
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            <span v-if="!complaint.admin_notified_at && complaint.status === 'pending'" class="new-badge">BARU</span>
                                        </div>
                                    </td>
                                    <td class="tbl-td">
                                    ...
                                    <td class="tbl-td max-w-[160px]">
                                        <p class="font-semibold text-sm text-gray-700 truncate group-hover:text-gray-900 transition-colors">{{ complaint.title }}</p>
                                    </td>
                                    <td class="tbl-td hidden md:table-cell">
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider bg-primary-50 text-primary-600 border border-primary-100">
                                            <span v-if="complaint.category?.parent" class="text-primary-400">{{ complaint.category.parent.name }} › </span>{{ complaint.category?.name }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="tbl-td text-xs hidden xl:table-cell min-w-[160px]">
                                        <template v-if="complaint.estimated_completion_date">
                                            <div class="flex justify-between items-end mb-1">
                                                <span v-if="isOverdue(complaint)" class="text-red-600 font-extrabold text-[10px] uppercase tracking-wider">⚠️ Terlewati</span>
                                                <span v-else-if="complaint.status === 'resolved' || complaint.status === 'rejected'" class="text-emerald-600 font-extrabold text-[10px] uppercase tracking-wider">✅ Selesai</span>
                                                <span v-else class="text-blue-600 font-extrabold text-[10px] uppercase tracking-wider">🕐 {{ remainingDays(complaint) }} Hari Lagi</span>
                                    ...
                                    <td class="tbl-td hidden lg:table-cell">
                                        <div v-if="complaint.rating" class="flex items-center gap-0.5">
                                            <svg v-for="star in 5" :key="star" class="w-3 h-3" :class="star <= complaint.rating ? 'text-amber-400' : 'text-gray-200'" fill="currentColor" viewBox="0 0 20 20">
                                    ...
                                    <td class="tbl-td text-xs text-gray-400 whitespace-nowrap hidden md:table-cell">{{ formatRelativeTime(complaint.created_at) }}</td>
                                    <td class="tbl-td text-right whitespace-nowrap" @click.stop>
                                        <div class="flex items-center justify-end gap-0.5">
                                            <button @click.stop="openDetail(complaint)" class="action-btn text-primary-500 hover:bg-primary-50" title="Lihat Detail">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                            </button>
                                            <button @click.stop="openUpdateModal(complaint)" class="action-btn text-amber-500 hover:bg-amber-50" title="Update Status">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                            </button>
                                            <button @click.stop="openDeleteModal(complaint)" class="action-btn text-red-400 hover:bg-red-50 hover:text-red-600" title="Hapus">
                ...
                <div v-else class="text-center py-24 px-6 relative group border border-transparent">
                    ...
                    <div class="relative z-10 transition-transform duration-500 group-hover:-translate-y-1">
                        <h3 class="text-2xl font-black text-primary-900 mb-3 tracking-tight group-hover:text-primary-700 transition-colors">{{ t('no_complaints_found') }}</h3>
                        <p class="text-primary-800/70 text-sm font-medium mb-8 max-w-sm mx-auto leading-relaxed group-hover:text-primary-800 transition-colors">{{ t('no_complaints_filter_desc') }}</p>
                        <button v-if="searchQuery || filterCategory || activeStatus !== 'all'" @click="searchQuery = ''; filterCategory = ''; activeStatus = 'all'" class="inline-flex items-center gap-2 bg-gradient-to-r from-primary-500 to-primary-600 text-white font-bold px-8 py-3.5 rounded-full hover:shadow-[0_8px_20px_-6px_rgba(124,58,237,0.5)] hover:-translate-y-1 transition-all duration-300">
                            {{ t('show_all') }}
                        </button>
                    </div>
                </div>
                ...
                <div class="flex gap-1.5">
                    <template v-for="(link, index) in complaints.links" :key="index">
                        <button
                            ...
                            v-html="link.label"
                        ></button>
                        <div v-else class="pagination-btn pagination-btn-disabled" v-html="link.label"></div>
                    </template>
                </div>
            </div>
        </div>
                    ...
                    <div class="detail-modal">
                        ...
                        <div class="flex items-start justify-between mb-6">
                            <div>
                                <div class="flex items-center gap-2 mb-1">
                                    <p class="text-[10px] font-bold text-primary-400 uppercase tracking-[0.15em]">LAPORAN › DETAIL LAPORAN</p>
                                </div>
                                <h2 class="text-xl font-extrabold text-gray-900 tracking-tight">Konsol Manajemen</h2>
                                <p class="text-xs text-gray-400 mt-0.5">Meninjau laporan <span class="font-bold text-primary-500">#RPT-{{ String(detailComplaint?.id).padStart(4, '0') }}</span></p>
                            </div>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div class="lg:col-span-2 space-y-5">
                                ...
                                <div class="bg-white/60 backdrop-blur-md border border-white/80 rounded-2xl p-5">
                                    <div class="flex items-start justify-between gap-4 mb-4">
                                        <div class="flex items-start gap-3">
                                            ...
                                            <div>
                                                <h3 class="text-base font-extrabold text-gray-900 leading-tight">{{ detailComplaint?.title }}</h3>
                                                <p class="text-xs text-gray-400 mt-0.5">{{ detailComplaint?.location }} <span v-if="detailComplaint?.location_detail">· {{ detailComplaint?.location_detail }}</span></p>
                                            </div>
                                        </div>
                                    ...
                                    <div class="grid grid-cols-3 gap-4 p-4 bg-gray-50/80 rounded-xl">
                                        ...
                                        <div>
                                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Dilaporkan</p>
                                            <p class="text-sm font-bold text-gray-800 mt-0.5">{{ formatDateShort(detailComplaint?.created_at) }}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <!-- Photos -->
                                <div v-if="detailComplaint?.image_paths?.length > 0" class="bg-white/60 backdrop-blur-md border border-white/80 rounded-2xl p-5">
                                ...
                                <div v-else-if="detailComplaint?.image_path">
                                    <div class="bg-white/60 backdrop-blur-md border border-white/80 rounded-2xl p-5">
                                        <p class="text-[10px] font-extrabold text-gray-400 uppercase tracking-widest mb-3">FOTO BUKTI</p>
                                        <img :src="resolveImageUrl(detailComplaint.image_path)" alt="Foto" class="max-h-52 rounded-xl object-cover border border-gray-200 cursor-zoom-in hover:opacity-90 transition-all duration-300 hover:scale-[1.02] shadow-sm" @click="openLightbox([resolveImageUrl(detailComplaint.image_path)], 0)" />
                                    </div>
                                </div>

                                <!-- Admin Response -->
                                <div v-if="detailComplaint?.admin_response" class="bg-primary-50/80 backdrop-blur-md border border-primary-100 rounded-2xl p-5">
                            ...
                            <div class="space-y-5">
                                ...
                                <div class="bg-white/60 backdrop-blur-md border border-white/80 rounded-2xl p-5">
                                    <h4 class="text-[10px] font-extrabold text-gray-900 uppercase tracking-widest mb-4">AKSI MANAJEMEN</h4>
                                ...
                                <!-- Estimation & Progress -->
                                <div v-if="detailComplaint?.estimated_completion_date" class="bg-white/60 backdrop-blur-md border border-white/80 rounded-2xl p-5">
                                    <h4 class="text-[10px] font-extrabold text-gray-900 uppercase tracking-widest mb-3">ESTIMASI PENYELESAIAN</h4>
                                    <div class="flex items-center justify-between mb-2">
                                        <span v-if="isOverdue(detailComplaint)" class="text-red-600 font-bold text-xs">⚠️ Terlewati</span>
                                        <span v-else-if="detailComplaint.status === 'resolved' || detailComplaint.status === 'rejected'" class="text-emerald-600 font-bold text-xs">✅ Selesai</span>
                                        <span v-else class="text-blue-600 font-bold text-xs">🕐 {{ remainingDays(detailComplaint) }} Hari Lagi</span>
                                    ...
                                    <p class="text-[10px] text-gray-500 font-medium">Target: <span class="font-bold text-gray-700">{{ formatEstDate(detailComplaint.estimated_completion_date) }}</span></p>
                                </div>

                                <!-- Rating -->
                                <div v-if="detailComplaint?.rating" class="bg-gradient-to-br from-amber-50 to-yellow-50 border border-amber-200 rounded-2xl p-5">
                                    <h4 class="text-[10px] font-extrabold text-amber-700 uppercase tracking-widest mb-3">⭐ RATING KEPUASAN</h4>
                                    <div class="flex items-center gap-1 mb-2">
                                        <svg v-for="star in 5" :key="star" class="w-5 h-5" :class="star <= detailComplaint.rating ? 'text-amber-400' : 'text-gray-200'" fill="currentColor" viewBox="0 0 20 20">
                                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                        </svg>
                                        <span class="text-sm font-bold text-amber-700 ml-1">{{ ratingLabel(detailComplaint.rating) }} ({{ detailComplaint.rating }}/5)</span>
                                    </div>
                                    <p v-if="detailComplaint.rating_comment" class="text-sm text-amber-800 italic leading-relaxed">"{{ detailComplaint.rating_comment }}"</p>
                                </div>
                                <div v-else-if="detailComplaint?.status === 'resolved'" class="bg-white/60 backdrop-blur-md border border-white/80 rounded-2xl p-5">
                                ...
                                <!-- Activity Timeline -->
                                <div class="bg-white/60 backdrop-blur-md border border-white/80 rounded-2xl p-5">
                                    <h4 class="text-[10px] font-extrabold text-gray-900 uppercase tracking-widest mb-4">AKTIVITAS TERKINI</h4>
                                    <div class="space-y-3">
                                        <div class="flex items-start gap-3">
                                            ...
                                            <div>
                                                <p class="text-xs font-bold text-gray-800">Laporan Dibuat</p>
                                                <p class="text-[10px] text-gray-400">{{ formatDate(detailComplaint?.created_at) }}</p>
                                            </div>
                                        </div>
                                        <div v-if="detailComplaint?.admin_response" class="flex items-start gap-3">
                                            ...
                                            <div>
                                                <p class="text-xs font-bold text-gray-800">Admin Merespons</p>
                                                <p class="text-[10px] text-gray-400">Respon telah diberikan</p>
                                            </div>
                                        </div>
                                        <div v-if="detailComplaint?.status !== 'pending'" class="flex items-start gap-3">
                                            <div class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5"
                                                ...
                                                }" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                                            </div>
                                            <div>
                                                <p class="text-xs font-bold text-gray-800">Status: {{ statusLabel[detailComplaint?.status] }}</p>
                                                <p class="text-[10px] text-gray-400">Diperbarui oleh admin</p>
                                            </div>
                                        </div>
                                        <div v-if="detailComplaint?.rating" class="flex items-start gap-3">
                                            ...
                                            <div>
                                                <p class="text-xs font-bold text-gray-800">Rating Diberikan: {{ detailComplaint.rating }}/5</p>
                                                <p class="text-[10px] text-gray-400">Oleh pelapor</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
        ...
        <Teleport to="body">
            <Transition name="modal" appear>
                <div v-if="showModal" class="modal-overlay" @click.self="showModal = false" style="background: rgba(15, 23, 42, 0.2);">
                    <div class="modal-content max-w-[500px] p-8 shadow-2xl relative" style="background: #efeff1; border: 1px solid rgba(255, 255, 255, 0.6); border-radius: 2rem;">
                        <!-- Header -->
                        <div class="flex items-start justify-between mb-6">
                            <div>
                                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1 shadow-sm">MANAJEMEN TUGAS</p>
                                <h3 class="text-[17px] font-extrabold text-gray-800 tracking-tight">Update Status Laporan</h3>
                            </div>
                            <button @click="showModal = false" class="p-1 hover:bg-gray-200/50 rounded-full transition text-gray-400 hover:text-gray-600">
                                <svg class="w-5 h-5 text-gray-500 font-bold" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12" /></svg>
                            </button>
                        </div>
                        ...
                        <form @submit.prevent="submitStatusUpdate" class="space-y-6">
                            <!-- Offline warning -->
                            <Transition name="net-err">
                                <div v-if="!isOnline" class="flex items-center gap-3 p-3 rounded-xl bg-red-50 border border-red-200 text-red-700 text-sm">
                                    <svg class="w-5 h-5 flex-shrink-0 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 5.636a9 9 0 010 12.728M15.536 8.464a5 5 0 010 7.072M12 12h.01M8.464 8.464a5 5 0 000 7.072M5.636 5.636a9 9 0 000 12.728" /></svg>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div class="relative">
                                        <select v-model="statusForm.status" class="w-full pl-4 pr-10 py-2.5 text-sm font-semibold text-gray-700 bg-white/90 border border-white rounded-xl shadow-sm appearance-none focus:ring-2 focus:ring-primary-500/30 outline-none transition-all">
                                            <option v-for="opt in statusOptions" :key="opt.value" :value="opt.value">{{ opt.label }}</option>
                                        </select>
                                        <svg class="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-primary-500 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7" /></svg>
                                    </div>
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        <input type="date" v-model="statusForm.estimated_completion_date" class="w-full pl-4 pr-10 py-2.5 text-sm font-semibold text-gray-700 bg-white/90 border border-white rounded-xl shadow-sm focus:ring-2 focus:ring-primary-500/30 outline-none transition-all" :min="new Date().toISOString().split('T')[0]" />
                                        <svg class="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-primary-500 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                                    </div>
                                    <InputError class="mt-1" :message="statusForm.errors.estimated_completion_date" />
                                </div>
                            </div>
                            ...
                            <div class="bg-white/80 backdrop-blur-sm rounded-[1.5rem] p-5 shadow-sm border border-white">
                                ...
                                <div class="relative pb-2">
                                    ...
                                    <p v-if="statusForm.status === 'resolved' || statusForm.status === 'rejected'" class="text-[9px] text-amber-600 mt-2 font-medium bg-amber-50/80 p-1.5 rounded-lg border border-amber-100/50">
                            ...
                            <div>
                                ...
                                ></textarea>
                                <InputError class="mt-1" :message="statusForm.errors.admin_response" />
                            </div>

                            <!-- Bukti Foto -->
                            <div v-if="selectedComplaint?.image_paths?.length > 0 || selectedComplaint?.image_path" class="flex gap-3 pt-1 pb-2">
                                <template v-if="selectedComplaint?.image_paths?.length > 0">
                                    <div v-for="(imgPath, idx) in selectedComplaint.image_paths" :key="idx" class="w-14 h-14 rounded-2xl overflow-hidden border-2 border-white shadow-sm shrink-0 relative flex items-center justify-center group cursor-zoom-in" @click="openLightbox(selectedComplaint.image_paths.map(p => resolveImageUrl(p)), idx)">
                                        <img :src="resolveImageUrl(imgPath)" class="w-full h-full object-cover group-hover:scale-110 transition duration-300" alt="Bukti Foto" />
                                    </div>
                                </template>
                                <template v-else-if="selectedComplaint?.image_path">
                                    <div class="w-14 h-14 rounded-2xl overflow-hidden border-2 border-white shadow-sm shrink-0 relative flex items-center justify-center group cursor-zoom-in" @click="openLightbox([resolveImageUrl(selectedComplaint.image_path)], 0)">
                                        <img :src="resolveImageUrl(selectedComplaint.image_path)" class="w-full h-full object-cover group-hover:scale-110 transition duration-300" alt="Bukti Foto" />
                                    </div>
                                </template>
                            </div>
                            ...
                            <div class="flex items-center justify-end gap-3 border-t border-gray-300/30 pt-5">
                                <button type="button" @click="showModal = false" class="px-7 py-2.5 rounded-full bg-white text-gray-600 font-extrabold text-xs hover:bg-gray-50 hover:text-gray-900 transition border border-gray-200 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)]">
                                    Batal
                                </button>
                                <button
                                    type="submit"
                                ...
                                >
                                    <svg v-if="statusForm.processing" class="animate-spin w-3 h-3 mr-2" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                    ...
                    <div class="modal-content max-w-sm p-6 rounded-2xl">
                        <div class="text-center">
                            <div class="relative w-20 h-20 mx-auto mb-6 group cursor-default">
                                <div class="absolute top-1 right-1 w-14 h-14 rounded-2xl bg-gradient-to-tr from-red-200 to-rose-200 shadow-sm transition-transform duration-300 group-hover:rotate-6 group-hover:scale-110"></div>
                                <div class="absolute bottom-1 left-1 w-16 h-16 rounded-2xl bg-white/60 backdrop-blur-md border border-white/80 flex items-center justify-center shadow-lg transition-transform duration-300 group-hover:-translate-y-1">
                                    <svg class="w-8 h-8 text-red-500 drop-shadow-sm" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                </div>
                            </div>
                            <h3 class="text-lg font-extrabold text-gray-900 mb-1">{{ t('delete_complaint') }}</h3>
                            <p class="text-sm text-gray-500 mb-1">{{ t('delete_confirm_desc') }}</p>
                            <div class="bg-gray-50 rounded-xl p-3 mb-6 border border-gray-100">
                                <p class="text-sm font-semibold text-gray-700 truncate">{{ deleteTarget?.title }}</p>
                            </div>
                            <div class="flex gap-3">
                                <button @click="submitDelete" class="flex-1 py-3 bg-red-500 text-white font-bold tracking-wide rounded-xl hover:bg-red-600 transition text-xs uppercase">
                                    {{ t('yes_delete') }}
                                </button>
                                <button @click="confirmDelete = false" class="flex-1 py-3 bg-gray-100 text-gray-700 font-bold tracking-wide rounded-xl hover:bg-gray-200 transition text-xs uppercase">
                                    {{ t('cancel') }}
                                </button>
                            </div>
                        </div>
        ...
        <Teleport to="body">
            <Transition name="lightbox">
                <div v-if="lightboxOpen" class="fixed inset-0 z-[200] flex items-center justify-center bg-gray-900/70 backdrop-blur-sm" @click.self="closeLightbox">
                    <button @click="closeLightbox" class="absolute top-4 right-4 w-10 h-10 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 text-white transition">✕</button>
                    <div v-if="lightboxImages.length > 1" class="absolute top-4 left-1/2 -translate-x-1/2 text-white/60 text-sm font-medium">{{ lightboxIndex + 1 }} / {{ lightboxImages.length }}</div>
                    <button v-if="lightboxImages.length > 1" @click="navLightbox(-1)" class="absolute left-4 w-12 h-12 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/25 text-white text-xl transition">‹</button>
                    <img :src="lightboxImages[lightboxIndex]" :alt="'Foto ' + (lightboxIndex + 1)" class="max-w-[90vw] max-h-[85vh] object-contain rounded-xl shadow-2xl select-none" style="animation: lightboxPop 0.35s cubic-bezier(0.22, 1, 0.36, 1);" />
                    <button v-if="lightboxImages.length > 1" @click="navLightbox(1)" class="absolute right-4 w-12 h-12 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/25 text-white text-xl transition">›</button>
                </div>
            </Transition>
        </Teleport>
    </AuthenticatedLayout>
