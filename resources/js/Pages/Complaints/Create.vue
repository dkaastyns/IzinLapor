...
<template>
  ...
  <Transition name="net-slide">
    <div
      v-if="fileSizePopup"
      class="fixed top-20 right-6 sm:top-8 sm:right-8 z-[200] flex items-center gap-4 p-4 pr-6 rounded-2xl bg-white/80 backdrop-blur-xl border border-red-100 shadow-[0_20px_40px_-10px_rgba(225,29,72,0.15)] animate-fade-in-up"
    >
      <div class="flex-shrink-0 w-12 h-12 rounded-full bg-gradient-to-tr from-red-100 to-rose-50 flex items-center justify-center border border-red-200">
        <svg
          class="w-6 h-6 text-red-500 drop-shadow-sm"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        ><path
      ...
      <div>
        <p class="font-extrabold text-gray-900 text-[15px] mb-0.5 tracking-tight">
          Upload Ditolak
        </p>
        <p class="text-xs text-gray-500 font-medium">
          Ukuran foto melebih batas maksimal <span class="font-bold text-red-500">1 MB</span>.
        </p>
      </div>
      <button
        class="ml-4 text-gray-400 hover:text-red-500 bg-gray-50 hover:bg-red-50 p-1.5 rounded-full transition-all"
        @click="fileSizePopup = false"
      >
        <svg
          class="w-4 h-4"
          fill="none"
    ...
    <template #header>
      <div class="flex items-center gap-4">
        <Link
          ...
          class="w-10 h-10 flex items-center justify-center rounded-full bg-white border border-gray-100 text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition shadow-sm"
        >
          <svg
    ...
    <div class="max-w-5xl mx-auto">
      ...
      <div class="complaint-form-card premium-panel border border-white/60 rounded-3xl overflow-hidden animate-fade-in-up transition-colors duration-300">
        ...
        <div class="text-center pt-10 pb-6 px-8">
          <h2 class="text-3xl sm:text-4xl font-black text-gray-900 tracking-tight mb-3">
            Buat Pengaduan
          </h2>
          ...
          >
            ...
            >
              ...
              >
                <!-- Completed Check -->
                <svg
                  v-if="isStepComplete(step.num) && currentStep !== step.num"
                  class="w-5 h-5"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
              ...
              <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
                ...
                <div class="space-y-6">
                  ...
                  <div>
                    <label
                      for="title"
                      class="block text-sm font-bold text-gray-800 mb-2.5"
                    >
                    ...
                    <div class="relative">
                      ...
                      >
                        ...
                        <optgroup label="── Kategori Utama ──">
                          <option
                            v-for="cat in categories.filter(c => !['Keamanan','Peralatan Elektronik','Lingkungan','Lainnya'].includes(c.name))"
                            :key="cat.id"
                            :value="cat.id"
                          >
                            {{ cat.name }}
                        ...
                        <optgroup label="── Kategori Tambahan ──">
                          <option
                            v-for="cat in categories.filter(c => ['Keamanan','Peralatan Elektronik','Lingkungan'].includes(c.name))"
                            :key="cat.id"
                            :value="cat.id"
                          >
                            {{ cat.name }}
                        ...
                        <optgroup label="── Lainnya ──">
                          <option
                            v-for="cat in categories.filter(c => c.name === 'Lainnya')"
                            :key="cat.id"
                            :value="cat.id"
                          >
                            {{ cat.name }}
                ...
                <div>
                  ...
                  <p
                    v-if="description.length > 0 && description.length < 20"
              ...
              <div class="max-w-2xl mx-auto">
                <div class="flex items-center justify-between mb-4">
                  ...
                  <span
                    ...
                    :class="imagePreviews.length >= 1 ? 'text-emerald-600 bg-emerald-50 border border-emerald-200' : 'text-gray-400 bg-gray-50 border border-gray-200'"
                  >
                ...
                >
                  ...
                  >
                    <div
                      ...
                      :key="item.preview"
                      class="flex items-center gap-4 p-3 bg-white/70 backdrop-blur-sm rounded-2xl border border-white/80 shadow-sm hover:shadow-md transition-all duration-300 group"
                    >
                      <img
                        :src="item.preview"
                        alt="Preview"
                      ...
                      <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-gray-400 font-medium">
                          {{ formatFileSize(item.size) }} · <span class="text-emerald-500 font-bold">Upload Berhasil</span>
                        </p>
                      </div>
                      <button
                        type="button"
                ...
                <p
                  v-if="imagePreviews.length === 0"
                  class="text-xs font-bold text-red-500 uppercase tracking-wider mt-3 flex items-center gap-1.5"
                >
                  <svg
              ...
              <div class="max-w-2xl mx-auto space-y-6">
                ...
                <Transition name="net-err">
                  ...
                  >
                    ...
                    <button
                      type="button"
                      class="flex-shrink-0 mt-0.5 hover:opacity-60 transition"
                ...
                <div
                  v-if="!canSubmit"
                  class="p-5 bg-white/50 backdrop-blur-sm rounded-2xl border border-white/70"
                >
                  ...
                  <div class="space-y-2 text-xs font-bold uppercase tracking-wide text-gray-400">
                    ...
                    >
                      <span class="w-5 h-5 rounded-full bg-red-50 text-red-400 flex items-center justify-center text-[10px]">✕</span> Pilih kategori utama
                    </p>
                    <p
                      v-if="category_id"
                      class="flex items-center gap-2"
                    >
                      <span class="w-5 h-5 rounded-full bg-emerald-50 text-emerald-500 flex items-center justify-center text-[10px]">✓</span> <span class="text-emerald-500">Kategori dipilih</span>
                    </p>
                    <p
                      v-if="hasSubCategories && !sub_category_id"
                      class="flex items-center gap-2"
                    >
                      <span class="w-5 h-5 rounded-full bg-red-50 text-red-400 flex items-center justify-center text-[10px]">✕</span> Pilih sub-kategori
                    </p>
                    <p
                      v-if="!title.trim()"
                      class="flex items-center gap-2"
                    >
                      <span class="w-5 h-5 rounded-full bg-red-50 text-red-400 flex items-center justify-center text-[10px]">✕</span> Isi judul laporan
                    </p>
                    <p
                      v-if="title.trim()"
                      class="flex items-center gap-2"
                    >
                      <span class="w-5 h-5 rounded-full bg-emerald-50 text-emerald-500 flex items-center justify-center text-[10px]">✓</span> <span class="text-emerald-500">Judul terisi</span>
                    </p>
                    <p
                      v-if="description.trim().length < 20"
                      class="flex items-center gap-2"
                    >
                      <span class="w-5 h-5 rounded-full bg-red-50 text-red-400 flex items-center justify-center text-[10px]">✕</span> Deskripsi minimal 20 karakter <span class="text-gray-300 normal-case ml-1">({{ description.trim().length }})</span>
                    </p>
                    <p
                      v-if="description.trim().length >= 20"
                      class="flex items-center gap-2"
                    >
                      <span class="w-5 h-5 rounded-full bg-emerald-50 text-emerald-500 flex items-center justify-center text-[10px]">✓</span> <span class="text-emerald-500">Deskripsi lengkap</span>
                    </p>
                    <p
                      v-if="imagePreviews.length === 0"
                      class="flex items-center gap-2"
                    >
                      <span class="w-5 h-5 rounded-full bg-red-50 text-red-400 flex items-center justify-center text-[10px]">✕</span> Upload minimal 1 foto bukti
                    </p>
                    <p
                      v-if="imagePreviews.length >= 1"
                      class="flex items-center gap-2"
                    >
                      <span class="w-5 h-5 rounded-full bg-emerald-50 text-emerald-500 flex items-center justify-center text-[10px]">✓</span> <span class="text-emerald-500">{{ imagePreviews.length }} foto diunggah</span>
                    </p>
                    <p
                      v-if="!location.trim()"
                      class="flex items-center gap-2"
                    >
                      <span class="w-5 h-5 rounded-full bg-red-50 text-red-400 flex items-center justify-center text-[10px]">✕</span> Pilih lokasi
                    </p>
                    <p
                      v-if="location.trim()"
                      class="flex items-center gap-2"
                    >
                      <span class="w-5 h-5 rounded-full bg-emerald-50 text-emerald-500 flex items-center justify-center text-[10px]">✓</span> <span class="text-emerald-500">Lokasi dipilih</span>
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div class="flex flex-col-reverse sm:flex-row items-center justify-between gap-4 mt-8 max-w-2xl mx-auto">
                ...
                >
                  ...
                  <div class="absolute inset-0 overflow-hidden rounded-2xl">
                    <div class="absolute top-0 -left-[150%] w-[80%] h-[300%] bg-gradient-to-r from-transparent via-white/20 to-transparent rotate-[35deg] animate-[shine-sweep_8s_ease-in-out_infinite]" />
                  </div>
                  <svg
                    v-if="processing"
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-0 divide-y sm:divide-y-0 sm:divide-x divide-gray-100/80">
            ...
            <div class="flex items-center gap-3.5 px-6 py-5 sm:py-6">
              ...
              <div>
                ...
                <p class="text-xs text-gray-500 font-medium leading-relaxed">
                  Data Anda akan diproses secara rahasia untuk keperluan perbaikan.
                </p>
              </div>
            </div>
            <!-- Respon Cepat -->
            <div class="flex items-center gap-3.5 px-6 py-5 sm:py-6">
              ...
              <div>
                ...
                <p class="text-xs text-gray-500 font-medium leading-relaxed">
                  Tim sarana prasarana akan menindaklanjuti dalam 1×24 jam kerja.
                </p>
              </div>
            </div>
            <!-- Status Realtime -->
            <div class="flex items-center gap-3.5 px-6 py-5 sm:py-6">
              ...
              <div>
                ...
                <p class="text-xs text-gray-500 font-medium leading-relaxed">
                  Dapatkan update status perbaikan langsung di dashboard Anda.
                </p>
              </div>
            </div>
          </div>
